...
const { registerValidation, loginValidation } = require("../middleware/validtion");
...
router.post("/loginPatient", async (req, res) => {
    ...
    if (error) return res.status(400).send(error.details[0].message);
    // checking if the healthID exists
    try {
        ...
        if (!patient) return res.status(400).send("HealthID is not found");
